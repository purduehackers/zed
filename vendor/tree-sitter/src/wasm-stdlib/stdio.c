...
static int vsnprintf_impl(char *buffer, size_t buffsz, const char *format, va_list args) {
  ...
  size_t capacity = buffsz > 0 ? buffsz - 1 : 0;
  ...
  while (*p) {
    if (*p == '%') {
      ...
      int width, precision;
      format_flags_t flags;
      p = parse_format_spec(p, &width, &precision, &flags, &args);
      if (!*p) break;
      ...
      switch (*p) {
        case 's': {
          ...
          // Wasm names are length-delimited, not necessarily NUL-terminated.
          // Avoid both an unbounded read and copying into the numeric buffer.
          output_len = 0;
          while ((precision < 0 || output_len < precision) && str[output_len]) output_len++;
          output_str = str;
          break;
        }
        case 'd':
      ...
      int str_len = output_len >= 0 ? output_len : strlen(output_str);
      int formatted_len = (width > str_len) ? width : str_len;
      total_chars += formatted_len;

      if (pos < capacity) {
        write_formatted_to_buffer(buffer, buffsz, &pos, output_str, str_len, width, &flags);
      }

    } else {
      if (pos < capacity) buffer[pos++] = *p;
      total_chars++;
    }
    p++;
  }
