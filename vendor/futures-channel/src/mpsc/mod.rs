...
// The browser main thread cannot enter std's futex wait when a worker holds a
// sender's short waker lock. Use the embedder's main-thread-aware parker there.
fn lock_sender(task: &Mutex<SenderTask>) -> MutexGuard<'_, SenderTask> {
    let guard = task.lock();
    #[cfg(not(all(target_arch = "wasm32", target_os = "unknown")))]
...
impl<T> BoundedSenderInner<T> {
    ...
    fn park(&mut self) {
        {
            let mut sender = lock_sender(&self.sender_task);
            sender.task = None;
    ...
    fn poll_unparked(&mut self, cx: Option<&mut Context<'_>>) -> Poll<()> {
        ...
        if self.maybe_parked {
            ...
            let mut task = lock_sender(&self.sender_task);
