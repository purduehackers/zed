...
use pet_python_utils::executable::{find_executables, should_search_for_environments_in_path};
...
/// as `is_file()`), shared with `resolve.rs` and kept here until pet-python-utils carries the
/// wasm gate itself.
#[cfg(target_family = "wasm")]
pub(crate) fn find_executable(env_path: &Path) -> Option<PathBuf> {
    [
