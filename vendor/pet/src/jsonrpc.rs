...
/// pet-core's `EnvironmentApi` has no `Environment` impl on wasm32-unknown-unknown; the
/// server reads the process environment through the library's `WasmEnvironmentApi` there.
#[cfg(target_family = "wasm")]
use pet::WasmEnvironmentApi as EnvironmentApi;
use pet_conda::Conda;
...
use pet_core::{
    os_environment::Environment, reporter::Reporter, Configuration, Locator,
    RefreshStatePersistence, RefreshStateSyncScope,
};
