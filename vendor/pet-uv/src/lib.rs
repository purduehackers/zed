...
/// cannot be handed to `Uv::from`. The browser build follows the Unix rules through
/// `WasmEnvironment` below, kept here until pet-core carries the wasm gate itself.
#[cfg(target_family = "wasm")]
impl Default for Uv {
    fn default() -> Self {
        Self::from(&WasmEnvironment)
    }
}

/// The Unix `EnvironmentApi` reduced to what `Uv::from` reads: `HOME` (normalised like
/// pet-core does) and named variables. The browser has no `PATH` to split, so the global
/// search locations are empty; nothing in this crate reads them.
#[cfg(target_family = "wasm")]
...
// wasm32-unknown-unknown is neither `unix` nor `windows`; the browser build follows the
// Unix rules (`symlink_metadata` fails there, so nothing is skipped).
#[cfg(any(unix, target_family = "wasm"))]
fn is_symlink_or_junction(path: &Path) -> bool {
    fs::symlink_metadata(path)
