...
// Browsers cannot park their main thread with memory.atomic.wait32. The embedder
// marks it for the D42-aware parking_lot_core parker; workers still park normally.
    ...
    Fut::Output: Clone,
{
    ...
    fn record_waker(&self, waker_key: &mut usize, cx: &mut Context<'_>) {
        ...
        #[cfg(any(not(feature = "std"), all(target_arch = "wasm32", target_os = "unknown")))]
...
// Native regression coverage lives separately to keep the upstream implementation diff small.
#[cfg(all(test, feature = "std", not(all(target_arch = "wasm32", target_os = "unknown"))))]
#[path = "shared_zs_tests.rs"]
mod zs_tests;
