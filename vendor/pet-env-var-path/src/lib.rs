...
fn normalize_search_path(path: PathBuf) -> PathBuf {
    // wasm32-unknown-unknown is neither `unix` nor `windows`; it follows the Unix rules
    // (`canonicalize` is unsupported there and fails, so the path comes back unchanged).
