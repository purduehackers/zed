...
#[cfg(not(target_family = "wasm"))]
use crate::{metadata, other, symlink_metadata};
...
///
/// On `wasm32-unknown-unknown` nothing can block inside `Drop`, so an unfinished
/// builder that is dropped writes no termination blocks, and the methods that read
/// the local filesystem (`append_path*`, `append_file`, `append_dir*`) do not exist.
///
/// [`into_inner`]: Builder::into_inner
/// [`finish`]: Builder::finish
pub struct Builder<W: Write + Unpin + Send + Sync> {
    // Read only by the local-filesystem methods, which are compiled out on wasm.
...
// On wasm `async_std::task::block_on` cannot block: it spawns a `'static` local task
// instead, which `&mut self` cannot outlive, so the builder is not finished on drop there.
#[cfg(feature = "runtime-async-std")]
#[cfg(not(target_family = "wasm"))]
