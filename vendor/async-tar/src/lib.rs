...
//!
//! On `wasm32-unknown-unknown` there is no filesystem, so everything that reads or
//! writes local files is compiled out: `Archive::unpack`, `Entry::unpack`,
//! `Entry::unpack_in`, `Builder::append_path` and the other path-, file- and
//! directory-based `Builder` methods, `Header::set_metadata` and
//! `Header::set_metadata_in_mode`. The archive reader, the `Header` parser and the
//! in-memory `Builder::append`/`Builder::append_data` are available everywhere.
...
// The filesystem helpers below, and everything that calls them, are compiled out on
// `wasm32-unknown-unknown`: `async_std::fs` (and the filesystem methods of
// `async_std::path::Path`) do not exist there.
