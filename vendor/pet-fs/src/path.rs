...
pub fn strip_trailing_separator<P: AsRef<Path>>(path: P) -> PathBuf {
    ...
    // wasm32-unknown-unknown is neither `unix` nor `windows`; the browser build follows
    // the Unix rules (its paths are `/`-separated).
    #[cfg(any(unix, target_family = "wasm"))]
    {
...
pub fn norm_case<P: AsRef<Path>>(path: P) -> PathBuf {
    ...
    // wasm32-unknown-unknown is neither `unix` nor `windows`; it is a no-op there too.
    #[cfg(any(unix, target_family = "wasm"))]
