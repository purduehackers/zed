...
/// `pet_python_utils::executable::find_executable` is `#[cfg(windows)]`/`#[cfg(unix)]`
/// upstream, so on wasm32-unknown-unknown (neither) it does not exist. The browser build
/// follows the Unix rules: this is that arm, kept here until pet-python-utils carries the
/// wasm gate itself.
#[cfg(target_family = "wasm")]
fn find_executable(env_path: &Path) -> Option<PathBuf> {
    [
...
/// `pet_core::os_environment::EnvironmentApi` implements `Environment` only under
/// `#[cfg(windows)]`/`#[cfg(unix)]` upstream, so on wasm32-unknown-unknown (neither) it
/// cannot stand in for `&dyn Environment`. The browser build follows the Unix rules: this
/// is that arm (`HOME`, `std::env::var`, a `:`-separated `PATH` plus `~/.local/bin`, kept
/// to the entries that exist), without the cached search-location list `EnvironmentApi`
/// keeps, and it lives here until pet-core carries the wasm gate itself. `Hatch` only asks
/// it for `HATCH_DATA_DIR`, `XDG_DATA_HOME` and the home directory.
#[cfg(target_family = "wasm")]
struct WasmEnvironmentApi;
...
/// The browser build's `Default`: `EnvironmentApi` has no `Environment` impl on
/// wasm32-unknown-unknown, so it reads the process environment through
/// `WasmEnvironmentApi` instead (same `HATCH_DATA_DIR` / home-directory rules).
#[cfg(target_family = "wasm")]
impl Default for Hatch {
    fn default() -> Self {
        Self::from(&WasmEnvironmentApi)
    }
}
