...
#[serde(rename_all = "camelCase")]
// Zed Codespaces: only ever built through `Deserialize`/`Default`, so rustc 1.97 reports it as
// never constructed; a path-sourced crate is compiled uncapped under CI's `-D warnings`.
#[allow(dead_code)]
