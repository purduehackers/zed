...
pub(crate) mod unix_file_path {
    ...
    /// url 2.5's `PATH_SEGMENT` and `SPECIAL_PATH_SEGMENT` (url/src/parser.rs:20-42), which
    /// `url` does not export.
    ...
    /// The Unix branch of `url::Url::from_file_path`, except that `.` and `..` components are
    /// rejected instead of being written raw into the URL.
    pub(crate) fn to_url(path: &Path) -> Result<url::Url, ()> {
        if !path.has_root() {
            return Err(());
        }
        ...
        for component in path.components() {
            match component {
                ...
                Component::Normal(segment) => {
                    ...
                    serialization.push('/');
                    serialization.extend(percent_encode(segment.as_bytes(), SPECIAL_PATH_SEGMENT));
                    pushed = true;
                }
            }
        }
    ...
    /// The Unix arm of `url::Url::to_file_path`: host must be `None` or `localhost` (no scheme
    /// check, as in `url`), every segment percent-decoded, non-UTF-8 rejected.
    pub(crate) fn from_url(url: &url::Url) -> Result<PathBuf, ()> {
        let segments = url.path_segments().ok_or(())?;
        match url.host() {
            None | Some(url::Host::Domain("localhost")) => {}
            _ => return Err(()),
        }
        let mut bytes = Vec::with_capacity(url.as_str().len());
        for segment in segments {
            bytes.push(b'/');
            bytes.extend(percent_decode(segment.as_bytes()));
        }
        // A windows drive letter must end with a slash (parity with `url`).
        if bytes.len() > 2
    ...
    #[cfg(test)]
    mod tests {
        use super::{from_url, to_url};
        use std::path::{Path, PathBuf};

        #[cfg(unix)]
        #[test]
        fn to_url_matches_url_from_file_path() {
            for path in [
                "/",
                "/a",
                "/a/b c",
                ...
                let expected = url::Url::from_file_path(path).unwrap();
                let actual = to_url(Path::new(path)).unwrap();
                assert_eq!(actual, expected, "path {path:?}");
            }
        }

        #[test]
        fn to_url_rejects_relative_and_parent_dir() {
            assert_eq!(to_url(Path::new("a/b")), Err(()));
            assert_eq!(to_url(Path::new("")), Err(()));
            assert_eq!(to_url(Path::new("/a/../b")), Err(()));
            assert_eq!(to_url(Path::new("/..")), Err(()));
        }
        ...
        #[cfg(unix)]
        #[test]
        fn from_url_matches_url_to_file_path() {
            for url in [
                "file:///home/u/a%20b",
                "file://localhost/x",
                "https://localhost/x",
                "file:///%C3%BCn%C3%AFcode/%E6%97%A5%E6%9C%AC",
                "file:///",
            ] {
                let url = url::Url::parse(url).unwrap();
                let expected = url.to_file_path();
                let actual = from_url(&url);
                assert_eq!(actual, expected, "url {url}");
            }
            assert_eq!(
                from_url(&url::Url::parse("https://localhost/x").unwrap()),
                Ok(PathBuf::from("/x"))
            );
        }

        #[test]
        fn from_url_rejects_hosts() {
            assert_eq!(from_url(&url::Url::parse("file://host/x").unwrap()), Err(()));
            assert_eq!(from_url(&url::Url::parse("mailto:x@y").unwrap()), Err(()));
        }

        #[test]
        fn roundtrip() {
            for path in ["/", "/a/b c", "/tmp/100%.txt", "/ünïcode/日本"] {
                let url = to_url(Path::new(path)).unwrap();
                assert_eq!(from_url(&url), Ok(PathBuf::from(path)), "path {path:?}");
            }
        }
    }
}
