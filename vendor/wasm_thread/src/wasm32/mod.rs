...
impl Builder {
    ...
    unsafe fn spawn_for_context(self, ctx: WebWorkerContext) {
        ...
        // Module workers are native in every browser that has the cross-origin-isolated
        // shared memory this crate needs (Chrome 80, Safari 15, Firefox 114), so the
        // `eval`-based polyfill upstream loads here is gone (it is what a `script-src`
        // CSP without `'unsafe-eval'` blocks).
