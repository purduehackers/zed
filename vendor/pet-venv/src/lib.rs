...
use pet_python_utils::executable::find_executable_or_broken;
#[cfg(target_family = "wasm")]
use pet_python_utils::executable::is_broken_symlink;
use pet_python_utils::executable::{find_executables, ExecutableResult};
use pet_python_utils::version;

/// `pet_python_utils::executable::find_executable_or_broken` is `#[cfg(windows)]`/`#[cfg(unix)]`
/// upstream, so on wasm32-unknown-unknown (neither) it does not exist. The browser build
/// follows the Unix rules: this is that arm (`bin/python`, `bin/python3`, `python`, `python3`,
/// then the same candidates again as broken symlinks), kept here until pet-python-utils
/// carries the wasm gate itself.
#[cfg(target_family = "wasm")]
fn find_executable_or_broken(env_path: &Path) -> ExecutableResult {
    let candidates = [
